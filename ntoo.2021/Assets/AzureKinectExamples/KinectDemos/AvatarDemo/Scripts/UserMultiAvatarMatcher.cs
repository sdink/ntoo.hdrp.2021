...
{
    public class UserMultiAvatarMatcher : MonoBehaviour
    {

        [Tooltip("Humanoid models used for avatar instatiation, one for each player index.")]
        ...
        private AvatarController CreateUserAvatar(ulong userId, int userIndex)
        {
            ...
            if (avatarModels != null && userIndex < avatarModels.Length && avatarModels[userIndex] != null)
            {
                ...
                GameObject avatarObj = Instantiate(avatarModels[userIndex], userPos, userRot);
