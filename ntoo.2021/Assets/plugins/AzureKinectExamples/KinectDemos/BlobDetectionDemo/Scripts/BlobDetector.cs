...
namespace com.rfilkov.components
{
    public class BlobDetector : MonoBehaviour
    {
        ...
        // initial camera rect
        private Vector2 cameraRectSize = Vector2.zero;
        ...
        void Update()
        {
            ...
            //// get image scales
            //float scaleX = backgroundImage ? Mathf.Abs(backgroundImage.rectTransform.localScale.x) : 1f;
            //float scaleY = backgroundImage ? Mathf.Abs(backgroundImage.rectTransform.localScale.y) : 1f;

            //// apply background rect size, too
            //Vector2 backRect = backgroundImage ? backgroundImage.rectTransform.rect.size : new Vector2(Screen.width, Screen.height);
            //if (cameraRectSize == Vector2.zero)
            //    cameraRectSize = backRect;

            ////Debug.Log("rectSize: " + backgroundImage.rectTransform.rect.size + ", offsetMin: " + backgroundImage.rectTransform.offsetMin + ", offsetMax: " + backgroundImage.rectTransform.offsetMax);
            ////Debug.Log("backRect: " + backRect + ", cameraRect: " + cameraRectSize);

            //scaleX *= backRect.x / cameraRectSize.x;
            //scaleY *= backRect.y / cameraRectSize.y;
            ...
            if (lastDepthFrameTime != sensorData.lastDepthFrameTime)  // kinectManager.GetDepthFrameTime(sensorIndex)
            {
                lastDepthFrameTime = sensorData.lastDepthFrameTime;  // kinectManager.GetDepthFrameTime(sensorIndex);
        ...
        {
            ushort[] rawDepth = sensorData != null ? sensorData.depthImage : null;  // kinectManager ? kinectManager.GetRawDepthMap(sensorIndex) : null;
            blobs.Clear();
