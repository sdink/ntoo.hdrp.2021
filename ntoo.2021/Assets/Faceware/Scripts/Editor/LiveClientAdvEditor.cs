    ...
    EditorGUILayout.BeginVertical();
    {
            ...
            if (FwLive.EnableRecording)
            {
                EditorGUILayout.BeginVertical("Box");
                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    {
                        GUILayout.Label(new GUIContent("Animation Clip:", "Animation clip that recorded animation will be saved to. If left blank, a new animation clip will be created at the root assets folder."), GUILayout.MinWidth(150));
                    ...
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        GUILayout.Label(new GUIContent("Character Setup File:", "Character Setup File used to tell the recorder which controls to record."), GUILayout.MinWidth(150));
                        FwLive.ExpressionSetFile = (LiveCharacterSetupFile)EditorGUILayout.ObjectField("", FwLive.ExpressionSetFile, typeof(LiveCharacterSetupFile), false);
                    }
                    EditorGUILayout.EndHorizontal();
...
#endif
                    FwLive.KeyframeOnNewData = GUILayout.Toggle(FwLive.KeyframeOnNewData, new GUIContent("Keyframe only on new Data", "Enable this feature to set keyframes on controls only when their values have changed"));
